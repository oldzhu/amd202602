"""
MXFP4-MM: Heuristic kernel selection experiment
Let the C++ get_heuristic_kernel pick both tile config AND splitK automatically.
No kernel name → full auto-select of best tile + splitK combo.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Pass empty kernel name + negative log2_k_split to trigger
    # C++ get_heuristic_kernel which picks best tile+splitK combo
    gemm_a4w4_asm(
        A_q, B_shuffle, A_scale_sh, B_scale_sh, out,
        "",  # empty → triggers heuristic
        bpreshuffle=True,
        log2_k_split=-1,  # negative → auto-select splitK
    )
    return out[:m, :]
