"""
Mixed-MLA: Direct ASM + kvgran16 + minimum-viable splits.
Key insight: total stage1 compute is constant regardless of splits.
Fewer splits = less reduce overhead. Only need enough splits for CU fill.
MI355X has 256 CUs. batch*16*splits >= 256 for full utilization.
"""
...
# Minimum viable splits: just enough for CU fill (batch*16*splits >= 256)
# batch=4: need splits>=4 for 256 blocks
# batch=32: 512 blocks with splits=1, but give 2 for some pipeline benefit
# batch=64+: 1024+ blocks, splits=1 is enough but 2 for safety
_SPLIT_MAP = {
    (4, 1024): 4,       # 256 blocks = exact CU fill, 16 KV chunks per split
    (4, 8192): 8,       # 512 blocks, 64 chunks per split
    (32, 1024): 2,      # 1024 blocks, 32 chunks per split
    (32, 8192): 4,      # 2048 blocks, 128 chunks per split
    (64, 1024): 2,      # 2048 blocks, 32 chunks per split
    (64, 8192): 4,      # 4096 blocks, 128 chunks per split
    (256, 1024): 2,     # 8192 blocks, 32 chunks per split
    (256, 8192): 4,     # 16384 blocks, 128 chunks per split
}
...
def custom_kernel(data: input_t) -> output_t:
    ...
    num_kv_splits = _SPLIT_MAP.get((batch_size, kv_seq_len), 4)
