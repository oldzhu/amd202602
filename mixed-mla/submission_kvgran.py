"""
Mixed-MLA: kv_granularity tuning experiment
Tests per-shape kv_granularity for better work distribution.
"""
...
def _get_metadata_workspace(
    ...
    num_kv_splits, kv_granularity, device,
):
    cache_key = (
        _device_cache_key(device), batch_size, max_q_len, nhead, nhead_kv,
        q_dtype, kv_dtype, num_kv_splits, kv_granularity,
    )
...
def _select_kv_granularity(batch_size, kv_seq_len):
    """Per-shape kv_granularity for optimal work distribution."""
    total_kv = batch_size * kv_seq_len
    if total_kv >= 1_000_000:
        return 32  # Large KV: coarser grain reduces metadata overhead
    return 16  # Default: fine-grained for smaller KV
...
def make_mla_decode_metadata(
    ...
    (
        ...
        batch_size, max_q_len, nhead, nhead_kv, q_dtype, kv_dtype,
        num_kv_splits, kv_granularity, qo_indptr.device,
    )
...
def custom_kernel(data: input_t) -> output_t:
    ...
    kv_granularity = _select_kv_granularity(batch_size, kv_seq_len)
