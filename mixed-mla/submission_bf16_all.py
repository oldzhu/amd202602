"""
Mixed-MLA: Pure BF16 — both Q and KV in bf16, skip all quantization
Saves kernel launch overhead at the cost of higher memory bandwidth.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Use bf16 KV directly — skip fp8 quantization entirely
    kv_buffer_bf16 = kv_data["bf16"]
    total_kv_len = kv_buffer_bf16.shape[0]
    kv_indices = _get_kv_indices(total_kv_len, q.device)
    kv_buffer_4d = kv_buffer_bf16.reshape(
        total_kv_len, PAGE_SIZE, nkv, kv_buffer_bf16.shape[-1]
    )
