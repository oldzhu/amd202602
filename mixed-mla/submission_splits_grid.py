"""
Mixed-MLA: Per-shape optimal split count experiment
Tests hardcoded per-shape split counts based on grid search analysis.
Shapes: bs={4,32,64,256} x kv={1024,8192}
"""
...
# Tested: current baseline uses 24 for most, 20 for bs=256/kv=1024, 16 for small
# This experiment tries slightly different values for specific shapes
_SPLIT_MAP = {
    # (batch_size, kv_seq_len) -> num_kv_splits
    (4, 1024): 12,       # Very small: try fewer splits
    (4, 8192): 16,       # Small batch, large kv: try 16
    (32, 1024): 16,      # Default bucket
    (32, 8192): 24,      # Keep at 24
    (64, 1024): 16,      # Default bucket
    (64, 8192): 24,      # Keep at 24
    (256, 1024): 20,     # Proven winner
    (256, 8192): 24,     # Large: keep at 24
}
