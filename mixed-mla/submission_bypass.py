"""
Mixed-MLA: Bypass mla_decode_fwd — call stage1 ASM + reduce directly.
Pre-allocates intermediate logits/attn_lse buffers to avoid per-call allocation.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Zero buffers to prevent stale data from previous calls
    logits.zero_()
    attn_lse.zero_()

    # Stage 1: ASM decode kernel (19 args, no final_lse)
    aiter.mla_decode_stage1_asm_fwd(
        ...
        kv_indices,
        kv_last_page_len,
        None,  # num_kv_splits_indptr (not needed for persistent)
        meta["work_meta_data"],
        meta["work_indptr"],
        meta["work_info_set"],
        max_q_len,
