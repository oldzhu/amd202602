"""
Mixed-MLA: Triton flash-decoding with bf16 Q + fp8 KV (cast in-kernel).
Combines fp8 bandwidth advantage with Triton's low dispatch overhead.
No FP8 quantization of Q needed (bf16 Q directly).
KV loaded as fp8 and cast to bf16 in registers before dot.
"""
...
@triton.jit
def _fwd_grouped_kernel_stage1_fp8(
    ...
    if split_kv_end > split_kv_start:
        for start_n in range(split_kv_start, split_kv_end, BLOCK_N):
            ...
            if BLOCK_DPE > 0:
                ...
                kpe_fp8 = tl.load(K_Buffer + offs_buf_kpe,
                    mask=(offs_n[None, :] < split_kv_end) & (mask_dpe[:, None]), other=0.0)
                kpe = kpe_fp8.to(tl.bfloat16)
                qk += tl.dot(qpe, kpe)
...
def _get_attn_logits(batch_size, num_heads, num_kv_splits, device):
    ...
    if cached is None:
        cached = torch.empty(
            (batch_size, num_heads, num_kv_splits, Lv_CONST + 1),
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # FP8 KV — halved bandwidth vs bf16
    kv_buffer_fp8, kv_scale = kv_data["fp8"]
    total_kv_len = kv_buffer_fp8.shape[0]

    # kv_buffer_fp8: (total_kv_len, nkv, 576) or (total_kv_len, 1, 576)
    if kv_buffer_fp8.dim() == 2:
        kv_buffer_fp8 = kv_buffer_fp8.unsqueeze(1)

    k_buffer = kv_buffer_fp8  # (total_kv_len, nkv, 576) fp8
    v_buffer = kv_buffer_fp8  # same buffer; kernel masks Lv=512
    ...
    _fwd_grouped_kernel_stage1_fp8[grid_stage1](
        ...
        logit_cap=0.0, Lk=Lk_CONST, Lv=Lv_CONST,
    ...
    _fwd_kernel_stage2[grid_stage2](
        ...
        NUM_KV_SPLITS=num_kv_splits, BLOCK_DV=BLOCK_DV, Lv=Lv_CONST,
