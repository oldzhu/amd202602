...
def _select_num_kv_splits(batch_size: int, kv_seq_len: int) -> int:
    """Use fewer splits on mid-sized decode batches while keeping the largest cases at 24 splits."""
    # More aggressive splits tuned for MI355X 304 CUs
    # bs * 16 heads = attention tasks; tasks * splits = work items
    # Target: enough work items for 304 CUs without excessive reduction overhead
    if total_kv >= 1_000_000:
        return 32  # large KV: max parallelism
    if batch_size >= 256:
        return 24
    if total_kv >= 131_072:
        return 24
