"""
Mixed-MLA: Direct stage1_asm + mla_reduce_v1 call, bypassing mla_decode_fwd wrapper.
- kv_granularity=32
- fp8 Q + fp8 KV
- Pre-allocate logits/attn_lse intermediate buffers
- Skip all wrapper overhead (type checks, reshaping, conditional logic)
"""
...
def _make_metadata(batch_size, max_q_len, kv_seq_len, nq, nkv, q_dtype, kv_dtype,
    ...
    # Pre-allocate logits and attn_lse for this shape
    total_s = batch_size * max_q_len
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Call stage1 ASM directly — persistent mode (num_kv_splits_indptr=None)
    mla_decode_stage1_asm_fwd(
        ...
        meta["work_indptr"],
        meta["work_info_set"],
        q_seq_len,  # max_seqlen_q
        PAGE_SIZE,
        nkv,  # nhead_kv
        SM_SCALE,
        meta["logits"],  # splitData (pre-allocated)
        meta["attn_lse"],  # splitLse (pre-allocated)
        o,  # output
        q_scale,
        kv_scale,
    )

    # Call reduce directly — compiled C++ kernel
    mla_reduce_v1(
        meta["logits"],
        meta["attn_lse"],
        meta["reduce_indptr"],
