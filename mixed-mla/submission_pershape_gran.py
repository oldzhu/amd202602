"""
Mixed-MLA: Per-shape kv_granularity — use larger granularity for bigger shapes.
(256,8192) is the bottleneck at 259μs. Try kv_granularity=64 or 128 for it.
"""
...
# Per-shape config: (splits, kv_granularity)
_SHAPE_CONFIG = {
    (4, 1024):   (12, 32),
    (4, 8192):   (16, 32),
    (32, 1024):  (16, 32),
    (32, 8192):  (24, 64),    # larger gran for more kv tokens
    (64, 1024):  (16, 32),
    (64, 8192):  (24, 64),    # larger gran
    (256, 1024): (20, 64),    # larger gran
    (256, 8192): (32, 128),   # max gran + max splits for bottleneck shape
}


def _make_metadata(batch_size, max_q_len, kv_seq_len, nq, nkv, q_dtype, kv_dtype,
                   qo_indptr, kv_indptr, kv_last_page_len, num_splits, kv_gran):
    key = (_device_key(qo_indptr.device), batch_size, max_q_len, kv_seq_len, nq, nkv,
           q_dtype, kv_dtype, num_splits, kv_gran)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    qo_indptr, kv_indptr, kv_last_page_len = _get_indptrs(batch_size, q_seq_len, kv_seq_len, q.device)
    ...
    num_kv_splits, kv_gran = _SHAPE_CONFIG.get((batch_size, kv_seq_len), (16, 32))

    meta = _make_metadata(
        batch_size, q_seq_len, kv_seq_len, nq, nkv,
        q_fp8.dtype, kv_buffer_4d.dtype,
        qo_indptr, kv_indptr, kv_last_page_len, num_kv_splits, kv_gran,
    )
