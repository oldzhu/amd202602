"""
Mixed-MLA: CUDA/HIP graph capture + direct ASM + kvgran16 + optimized splits.
Captures the kernel launch sequence (stage1_asm + mla_reduce) as a graph.
Eliminates per-call Python overhead and kernel launch overhead.
...
# Optimized splits: s=4 for batch=256 (from top entries), moderate for others
_SPLIT_MAP = {
    (4, 1024): 16,
    (4, 8192): 32,
    (32, 1024): 16,
...
def _run_kernels(q_reshaped, kv_buffer_4d, qo_indptr, kv_indptr, kv_indices,
                 kv_last_page_len, meta, q_seq_len, nkv, q_scale, kv_scale, o):
...
def custom_kernel(data: input_t) -> output_t:
    ...
    num_kv_splits = _SPLIT_MAP.get(key, 16)
    ...
    if count < 2:
        _run_kernels(q_reshaped, kv_buffer_4d, qo_indptr, kv_indptr, kv_indices,
                     kv_last_page_len, meta, q_seq_len, nkv, q_scale, kv_scale, o)
        return o
    ...
    if key not in _GRAPH_CACHE:
        try:
            ...
            with torch.cuda.graph(g):
                _run_kernels(q_reshaped, kv_buffer_4d, qo_indptr, kv_indptr,
                             kv_indices, kv_last_page_len, meta, q_seq_len,
                             nkv, q_scale, kv_scale, o)
            _GRAPH_CACHE[key] = g
            _GRAPH_OUTPUT_CACHE[key] = o
            return o
        except Exception:
    ...
    # Subsequent calls: replay or fallback
    ...
    graph.replay()
    return _GRAPH_OUTPUT_CACHE[key]
